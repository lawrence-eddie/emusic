...
<div class="container-xl mt-3">
  <div class="row">
    <?php
    $queryPopularSongs = mysqli_query($dbcon, "SELECT video_id, artist_id, video_name, video_pic, tagged_artist, artist_name FROM video
    ...
    if ($queryPopularSongsNum >= 3) {
      ...
      while ($fetch_popularSongs = mysqli_fetch_assoc($queryPopularSongs)) {
        // Audio Details
        $video_id = $purifier->purify($fetch_popularSongs['video_id']);
        $artist_id = $purifier->purify($fetch_popularSongs['artist_id']);
        $video_name = $purifier->purify($fetch_popularSongs['video_name']);
        $video_pic = $purifier->purify($fetch_popularSongs['video_pic']);
        ...
        $popAudio_name = "$video_name $popTagged_artist";
        $hit_link = "video.php?mid=$video_id&aid=$artist_id";
        ...
        echo "<div class='carousel-item allSongsCaro-item w-100 $queryPopularSongsNum'>
          ...
          <img src='$video_pic' class='d-block w-100 img-fluid' alt='$video_name'>
      ...
      <div class="row">
        ...
        <div class="col-lg-8 all-songs mb-3 mb-lg-5">
          ...
          try {
            ...
            $result_count = mysqli_query($dbcon, "SELECT COUNT(blog_id) As total_records FROM `music_blog` WHERE mime_type='video' AND display_track='yes'");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; // total page minus 1

            $queryMusicBlog = "SELECT * FROM music_blog WHERE mime_type='video' ORDER BY date_added DESC LIMIT ?, ?";
            ...
            if (mysqli_num_rows($queryBlog_result) > 0) {
              ...
              echo '<div class="card">
                <div class="card-body">
                  There are currently no Video check back later!.
                </div>
              </div>';
            }
