...
$artist_details = mysqli_query($dbcon, "SELECT vd.*, au.audio_name, au.audio_file, au.audio_pic, au.tagged_artist, au.lyrics,
at.artist_name, ab.album_name FROM video vd INNER JOIN audio au ON vd.artist_id=au.artist_id AND vd.audio_id=au.audio_id
INNER JOIN artist at ON vd.artist_id=at.artist_id LEFT JOIN album ab ON au.album_id=ab.album_id WHERE vd.video_id=$mid AND vd.artist_id=$aid");
// Check the result:
if (mysqli_num_rows($artist_details) == 1) {
  // fetch the records
  $fetch_artist_details = mysqli_fetch_assoc($artist_details);
  // Audio Details
  $audio_id = $purifier->purify($fetch_artist_details['audio_id']);
  $vid_name = $purifier->purify($fetch_artist_details['video_name']);
  $vid_file = $purifier->purify($fetch_artist_details['video_file']);
  $youtube_url = $purifier->purify($fetch_artist_details['youtube_url']);
  $vid_pic = $purifier->purify($fetch_artist_details['video_pic']);
  ...
  $vid_file = (empty($vid_file)) ? $adminPicture : "$vid_file";
  $vid_pic = (empty($vid_pic)) ? $adminPicture : "$vid_pic";
  $audio_file = (empty($audio_file)) ? $adminPicture : "$audio_file";
...
if (isset($artist_name)) {
  $page_title = "$artist_name $tagged_artist - $vid_name";
}
      ...
      <div class="card mt-3" id="audio_section">
        ...
        <div class="card-body">
          ...
          <div class="music-download">
            ...
            echo "<p class='card-text mb-0 mt-3 font-italic' id='song-download-btn' data-id='$audio_id'><small><a href='download_files.php?file=$download_file' class='text-primary'>
  ...
  <div class="row mt-5">
    <div class="col-lg-8">
      ...
      if ($query_RelatedSongsnumb >= 3) {
        ...
        echo "<div class='jumbotron-fluid py-3 float-app border'>
        <h6 class='text-muted pl-3 font-weight-bold text-muted'>Other song by $artist_name</h6>
        <div class='rel_song-carousel mx-auto justify-content-center col-sm-12 py-3' data-aos='zoom-in' data-aos-duration='1000'>";
        // fetch the records
        while ($fetch_RelatedSongs = mysqli_fetch_assoc($query_RelatedSongs)) {
          // Audio Details
      ...
      <div class="" id="comment-sect">
        ...
        if (!isset($user_id)) {
          ...
          echo "<div class='expanding-wrapper p-1'>
            ...
            placeholder='Write a comment...' rows='1' data-artist='$aid' data-video='$mid'></textarea>
          </div>";
        }
        ?>
          ...
          document.addEventListener('DOMContentLoaded', function() {
            $("textarea.audio_commentxt").keypress(function(e) {
              if (!$.trim($(this).val()).length < 1 && e.which == 13 && !e.shiftKey) {
                ...
                var data_video_id = $(this).attr("data-video");
                ...
                $.ajax({
                  ...
                  data: {
                    vidCommenTxt: val,
                    vidArtist_id: data_artist_id,
                    video_id: data_video_id
                  },
                  success: function(comments) {
                    // Update comments
                    $("ul#comment_section_video").prepend(comments);
                    if (!$.trim($('ul#comment_section_video').html()).length) {
                      location.reload()
                    }
                    $("ul#comment_section_video").load(window.location.href + " ul#comment_section_video");
                  }
                });
                $(this).val("");
              }
        ...
        avc.app_id=$app_id AND avc.video_id=$mid AND avc.artist_id=$aid ORDER BY comment_id DESC LIMIT 20");
        // Check the result:
        $app_com_num = mysqli_num_rows($app_comment);
        if ($app_com_num > 0) {
          echo "<ul class='list-unstyled ul-comment' id='comment_section_video'>";
...
<script type="text/javascript">
  ...
  $("#song-download-btn").click(function() {
    //ajax, increment number of downloads
    var data_song_id = $(this).attr("data-id");
    // alert(data_song_id);
    $.ajax({
      url: 'au_vid_comment.php',
      type: 'post',
      data: {
        song_id: data_song_id
      },
